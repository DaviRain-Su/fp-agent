...
let scaffold ?id ?tool_name ?(kind = "read") ?(template = "shell") dir =
  ...
  match
    ( validate_plugin_id id,
      validate_name ~what:"tool name" ~allow_dot:false tool_name,
      kind_of_string kind,
      scaffold_template_of_string template )
  with
  | Error e, _, _, _ | _, Error e, _, _ | _, _, Error e, _ | _, _, _, Error e ->
      Error e
  | Ok (), Ok (), Ok tool_kind, Ok template -> (
      ...
      let script_path = Stdlib.Filename.concat dir template.script_name in
...
|}
                   id id supported_sdk_version tool_name kind template.command
                   permissions));
