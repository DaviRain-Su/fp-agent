...
let test_write_then_read () =
  with_workspace (fun ws _ ->
      let w =
        run ws (Tool_call.write_file ~path:"a/b.txt" ~content:"hello world")
      in
      Alcotest.(check bool) "write ok" true (is_success w);
      let r = run ws (Tool_call.read_file "a/b.txt") in
      Alcotest.(check string) "read back" "hello world" (output r))

let test_edit () =
  with_workspace (fun ws _ ->
      ignore (run ws (Tool_call.write_file ~path:"f.txt" ~content:"let x = 42"));
      let e =
        run ws (Tool_call.edit_file ~path:"f.txt" ~old_text:"42" ~new_text:"43")
      in
      Alcotest.(check bool) "edit ok" true (is_success e);
      let r = run ws (Tool_call.read_file "f.txt") in
...
let test_edit_no_match () =
  with_workspace (fun ws _ ->
      ...
      let e =
        run ws (Tool_call.edit_file ~path:"f.txt" ~old_text:"zzz" ~new_text:"q")
      in
...
let test_list_files () =
  with_workspace (fun ws _ ->
      ...
      let l = run ws (Tool_call.list_files ".") in
      Alcotest.(check bool)
        "lists files" true
...
let test_search () =
  with_workspace (fun ws _ ->
      ignore
        (run ws
           (Tool_call.write_file ~path:"a/foo.ml"
              ~content:"let needle = 1\nlet x = 2"));
      ignore
        (run ws (Tool_call.write_file ~path:"b/bar.ml" ~content:"let y = 3"));
      let r = run ws (Tool_call.search "needle") in
      Alcotest.(check bool)
      ...
      let none = run ws (Tool_call.search "zzzznope") in
      Alcotest.(check bool)
      ...
      let r =
        run ws
          (Tool_call.multi_edit
             [ ("a.txt", "one", "1"); ("b.txt", "two", "2") ])
      in
      Alcotest.(check bool) "multi_edit ok" true (is_success r);
      Alcotest.(check string)
        "a edited" "1"
        (output (run ws (Tool_call.read_file "a.txt")));
      Alcotest.(check string)
        "b edited" "2"
        (output (run ws (Tool_call.read_file "b.txt"))))
...
let test_run_command () =
  with_workspace (fun ws _ ->
      let r = run ws (Tool_call.run_command "echo hi") in
      Alcotest.(check bool)
        "command output" true
...
let test_policy_denied () =
  with_workspace (fun ws _ ->
      ...
      let c = run ws (Tool_call.run_command "rm -rf /") in
...
let test_event_log () =
  with_workspace (fun _ root ->
      ...
      Event_log.append log (Event.Tool_call (Tool_call.read_file "f.txt"));
